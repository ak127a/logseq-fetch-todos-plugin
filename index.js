...
(() => {
  ...
  var SessionStateSchema = external_exports.object({
    ...
    hasInitialFocus: external_exports.boolean()
  });
  function createSessionState(sourceBlockUuid) {
  ...
  function updateSessionState(current, patch) {
    return SessionStateSchema.parse({
      ...current,
      ...patch
    });
  ...
  function syncDebugLoggingSetting(settings = logseq.settings) {
  ...
  function blockEscapeCloseTemporarily() {
    escapeCloseBlockedUntil = Date.now() + ESCAPE_CLOSE_GUARD_MS;
  }
  function isEscapeCloseBlocked() {
  ...
  function setSessionPatch(sessionId, patch) {
    const current = sessions.get(sessionId);
    if (!current) {
      return null;
    }
  ...
  function bindKeyboardListeners() {
    ...
    document.addEventListener("keydown", (event) => {
      ...
      const modal = document.querySelector(`.fts-modal[data-session-id='${session.sessionId}']`);
      if (!modal) {
        return;
      }
      if (event.key === "Escape") {
  ...
  function setTodoItemSelectedState(itemElement, selected) {
    itemElement.classList.toggle("is-selected", selected);
    itemElement.setAttribute("aria-checked", selected ? "true" : "false");
    const checkElement = itemElement.querySelector(".fts-check");
    if (checkElement) {
      checkElement.textContent = selected ? "\u2713" : "";
    }
  }
  function syncSelectionSummary(sessionId) {
    const session = getSession(sessionId);
    ...
    for (const countElement of Array.from(document.querySelectorAll(`${rootSelector} .fts-count`))) {
  ...
  function syncVisibleTodoSelectionState(sessionId) {
    const session = getSession(sessionId);
    if (!session || session.status !== "ready" || activeSessionId !== sessionId) {
      return;
    }
    const selectedSet = new Set(session.selectedIndices);
    for (const itemElement of Array.from(document.querySelectorAll(`.fts-item[data-session-id='${sessionId}']`))) {
  ...
  function renderTodoSelector(sessionId) {
    const session = getSession(sessionId);
  ...
  function extractPageReference(content) {
  ...
  async function getReferencedPageName(currentBlock) {
